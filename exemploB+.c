...
// Default order
#define ORDER 5 //order is 2d+1, so the assignement defines order as 2 2d+1 = 5
...
record *makeRecord(int value, char *name);
node *makeNode(void);
node *makeLeaf(void);
...
node *insert(node *root, int key, int value, char *name);
...
node *insert(node *root, int key, int value, char name[100]) {
  ...
  record_pointer = makeRecord(value, name);
...
int main() {
  ...
  root = insert(root, 5, 33, "dwarf");
  root = insert(root, 15, 21, "andre");
  root = insert(root, 25, 31, "deniro");
  root = insert(root, 35, 41, "vitata");
  root = insert(root, 45, 10, "maya");
  root = insert(root, 46, 11, "andrea");
  root = insert(root, 47, 12, "andrea");
  root = insert(root, 50, 15, "hoje");
  root = insert(root, 51, 16, "amanda");

  printTree(root);

  findAndPrint(root, 45, false);
